        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (!filePart.getSubmittedFileName().isEmpty()) {
                ...
                if (imagePathSave != null) {
                    if (PDAO.createProduct(productName, supplierID, categoryID, unitPrice, imagePathSave, description) > 0) {
                        response.sendRedirect("MainController?action=listProductAdmin");
                    } else {
                        sendError(request, response, "Cannot create new product!!","listProductAdmin");
                    }
                } else {
                    sendError(request, response, "Something wrong with product image","listProductAdmin");
                }
            }else{
                sendError(request, response, "Image required!!","listProductAdmin");
            } 
